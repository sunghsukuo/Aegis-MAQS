    ...
    def fetch_etf_constituents(self, etf_ticker: str) -> list:
        ...
        try:
            ...
            with db.db_session() as conn:
                ...
                row = cursor.fetchone()
                if row:
                    sector_id = row[0] if isinstance(row, (tuple, list)) else (row.get("id") if isinstance(row, dict) else row["id"])
                    db.execute_sql(cursor,
                    ...
                    if rows:
                        db_constituents = [r[0] if isinstance(r, (tuple, list)) else (r.get("ticker") if isinstance(r, dict) else r["ticker"]) for r in rows]
                ...
                if strat == "reversion":
                    if is_zh:
                        lines.append(
                            ...
                            "> 適用於市場震盪整理或具備均值回歸特性的板塊。系統先排除「長期空頭（低於 200MA）」與「短線過度延伸（偏離 50MA 超過 -5% 或 +8%）」的個股，並要求 RSI-14 <= 55。\n"
                            "> 通過篩選後，量化評分由以下四個維度加總計算（總分越高越推薦）：\n"
                            "> 1. **拉回幅度分數**：近 5 日股價跌幅百分比（跌越多分數越高）。\n"
                            "> 2. **RSI 輔助分數**：(100 - RSI-14) / 10（越超賣分數越高，滿分 10 分）。\n"
                            "> 3. **均線支撐加成**：股價位於 50MA 之上 `0.0% ~ 4.0%` 區間給予 **`+5.0` 分**；位於 50MA 之下 `0.0% ~ -3.0%` 區間給予 **`+3.0` 分**。\n"
                            "> 4. **成交量加成**：min(當前量能比, 2.0) / 2 × 3.0（量能出現低接信號時，滿分 3 分）。\n"
                        )
                    else:
                        lines.append(
                            "> [!NOTE]\n"
                            "> **Mean Reversion Strategy Scoring Explanation**:\n"
                            "> Used for rangebound markets or mean-reverting sectors. The system filters out \"long-term downtrends (below 200MA)\", \"short-term overextended price (deviated from 50MA by <-5% or >+8%)\", and requires RSI-14 <= 55.\n"
                            ...
                            "> 2. **RSI Score**: (100 - RSI-14) / 10 (more oversold yields higher score, max 10 points).\n"
                            "> 3. **Support Bonus**: **`+5.0` points** if price is `0.0% ~ 4.0%` above 50MA; **`+3.0` points** if price is `0.0% ~ -3.0%` below 50MA.\n"
                            "> 4. **Volume Score**: min(Volume Spike, 2.0) / 2 × 3.0 (rewards low-level accumulation volume, max 3 points).\n"
                        )
                elif strat == "momentum":
                    if is_zh:
                        lines.append(
                            ...
                            "> 1. **多頭上漲/正常情境**：5日漲幅百分比 × 0.7 + 成交量得分 × 0.3（其中，成交量得分 = min(成交量增幅, 3.0) / 3.0 × 10，注重漲幅動能）。\n"
                            "> 2. **震盪整理情境 (VOLATILE_RANGEBOUND)**：5日漲幅百分比 × 0.5 + 成交量得分 × 0.5 - 波動率懲罰（其中，成交量得分 = min(成交量增幅, 3.0) / 3.0 × 10，波動率懲罰為 min(20日日波動率 × 0.75, 2.5)）。\n"
                            "> 3. **熊市避險情境 (BEAR_RISK_OFF)**：5日漲幅百分比 × 0.3 + 成交量得分 × 0.7 - 波動率懲罰（其中，成交量得分 = min(成交量增幅, 3.0) / 3.0 × 10，波動率懲罰為 min(20日日波動率 × 1.5, 5.0)）。\n"
                        )
                    else:
                        lines.append(
                            "> [!NOTE]\n"
                            ...
                            "> 1. **Bull/Normal Regime**: 5-day Return % × 0.7 + Volume Spike Score × 0.3 (rewards strong upward price momentum).\n"
                            "> 2. **Volatile Rangebound Regime**: 5-day Return % × 0.5 + Volume Spike Score × 0.5 - Volatility Penalty (where Volume Spike Score = min(Volume Spike, 3.0) / 3.0 × 10, and penalty is min(20-day Volatility % × 0.75, 2.5)).\n"
                            "> 3. **Bear Risk Off Regime**: 5-day Return % × 0.3 + Volume Spike Score × 0.7 - Volatility Penalty (where Volume Spike Score = min(Volume Spike, 3.0) / 3.0 × 10, and penalty is min(20-day Volatility % × 1.5, 5.0)).\n"
                        )
            lines.append("\n---")
