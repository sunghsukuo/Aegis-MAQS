...
2. 所有評級為 Hold（持有/觀望）或 Sell / Avoid（避免買入）的標的，**必須無一遺漏地**列入「### 本週調降/避險排除標的與防禦配置說明」表格。**為了加強決策的對比與因果綁定，此表格必須包含以下列：| 國家區域 | 標的代碼 | 企業名稱 | 原始評級 | 最終評級 | 隸屬板塊/ETF | 決策防線聯動機制狀態 | 不推薦/調降原因 |。請在「原始評級」、「最終評級」與「決策防線聯動機制狀態」欄位中，根據上方候選標的分析說明，精確填入該標的對應的對照狀態與防線狀態（例如台積電原始為「避免買入 Avoid」、最終為「持有 Hold」、防線狀態為「【降級防禦】動能/主題軌道...」），讓讀者能一目了然地知道該標的評級前後的對比變化以及背後具體的風控判定因果！**
